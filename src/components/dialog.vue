...
//向父组件传递信息
const emit =
  defineEmits<{ (e: "visibleChange"): void; (e: "addUserRefresh"): void }>();
//传递diglog显示消失布尔值
...
const confirm = async function () {
  ...
  if (d.AddDiglogShow === true) {
    ...
    const AlterRes = (
      await AlterUser(AlterRuleForm.value.id, {
        email: AlterRuleForm.value.email,
        mobile: AlterRuleForm.value.mobile,
      })
    ).data;
    ElMessage({
